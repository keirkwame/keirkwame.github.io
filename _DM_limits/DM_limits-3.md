---
title: "Sub-GeV dark matter (proton-DM cross section)"
excerpt: "Limits (two sigma) on the proton-DM cross section vs. particle mass.<br/><img src='/images/bDM_cosmo.png'>"
collection: DM_limits
---

CMB = Cosmic microwave background (Planck)<br/>Lyaf (SDSS-I) = Lyman-alpha forest (Sloan Digital Sky Survey I)<br/>Lyaf (HIRES/UVES) = Lyman-alpha forest (Keck-HIRES; VLT-UVES)<br/>MW satellites = Milky Way sub-halo mass function<br/>XQC = X-ray Quantum Calorimeter<br/>Direct detection = CRESST-surface; EDELWEISS-Surf<br/><br/>BibTex for references and public code to follow.
